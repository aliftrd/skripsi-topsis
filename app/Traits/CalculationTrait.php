...
trait CalculationTrait
{
    ...
    */
    public function processCalculation($citizens, $criterias)
    {
        ...
        return compact('normalizeMatrix', 'matrixRij', 'normalizeMatrixWeight', 'minMax', 'idealMinMaxDistance', 'preferenceValue', 'citizenRanks');
    }
    ...
    private function preferenceValue(array $idealMinMaxDistance): array
    {
        ...
        foreach ($idealMinMaxDistance['plus'] as $citizenCode => $value) {
            $plusValue = $idealMinMaxDistance['plus'][$citizenCode] ?? 0;
            $minusValue = $idealMinMaxDistance['minus'][$citizenCode] ?? 0;

            if ($plusValue == 0 && $minusValue == 0) {
                $matrix[$citizenCode] = 0;
            } elseif ($plusValue == 0) {
                $matrix[$citizenCode] = $minusValue;
            } else {
                $matrix[$citizenCode] = round(($minusValue / $plusValue) + $minusValue, 3);
            }
        }
